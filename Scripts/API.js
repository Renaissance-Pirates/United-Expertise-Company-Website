...
const Send_Email = async (Language) =>
{
	...
	document.getElementsByClassName ('Contact_Us_Response').classList.remove ('Display_None');
	if (Response_Body.Status === 201)
	{
		document.getElementById ('Contact_Us_Header').classList.add ('Display_None');
		document.getElementsByClassName ('Contact_Us_Form').classList.add ('Display_None');
